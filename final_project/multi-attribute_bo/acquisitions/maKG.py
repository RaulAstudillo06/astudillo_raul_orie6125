...
class maKG(AcquisitionBase):
    ...
    def _compute_acq(self, X, parallel=False):
        ...
        :param X: points at which the acquisition function is evaluated. Should be a 2d array.
        """
        full_support = True # If true, the full support of the utility function distribution will be used
                            # when computing the acquisition function value.
        #X =np.atleast_2d(X)
        if full_support:
    ...
    def _marginal_acq(self, X, utility_params_samples):
        """
        Computes the marginal value of information for each of the utility parameters samples.
        """
        ...
        #np.random.seed(11)
        Z_samples = [i/n_z for i in range(n_z)] #np.random.normal(size=n_z)
    ...
    def _marginal_acq_parallel(self, X, utility_params_samples):
        """
        """
        pass
    ...
    def _compute_acq_withGradients(self, X):
        """
        Computes the acquisition function and its gradient.
        """
    ...
    def _marginal_acq_with_gradient(self, X, utility_params_samples):
        """
        Computes the marginal value of information and its gradient for each of the utility parameters samples.
        """
        marginal_acqX = np.zeros((X.shape[0],len(utility_params_samples)))
